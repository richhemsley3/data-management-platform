...
### Status Token Compliance
Verify these DMP states use the correct status tokens:

| State | Expected Token | Check |
|-------|---------------|-------|
| Connection active | `--sds-status-success-*` | Green badge |
| Connection error | `--sds-status-error-*` | Red badge |
| Scan running | `--sds-status-info-*` | Blue indicator |
| Classification pending | `--sds-status-warning-*` | Yellow badge |
| Classification confirmed | `--sds-status-success-*` | Green badge |
| Risk: Low (0-25) | `--sds-status-success-*` | Green |
| Risk: Moderate (26-50) | `--sds-status-warning-*` | Yellow |
| Risk: High (51-75) | `--sds-status-error-*` | Orange/red |
| Risk: Critical (76-100) | `--sds-status-error-*` | Red |
| Remediation applied | `--sds-status-success-*` | Green |
| Remediation failed | `--sds-status-error-*` | Red |
| Policy active | `--sds-status-success-*` | Green |
| Policy draft | `--sds-status-neutral-*` | Gray |
| Policy disabled | `--sds-status-warning-*` | Yellow |
| Regulation compliant | `--sds-status-success-*` | Green |
| Regulation non-compliant | `--sds-status-error-*` | Red |
| Regulation partial | `--sds-status-warning-*` | Yellow |
...
### Terminology
- [ ] UI says "Connection" not "Data source"
- [ ] UI says "Classification" not "Sensitivity label"
- [ ] UI says "Risk score" not "Risk metric"
- [ ] UI says "Remediation" not "Protection action"
- [ ] UI says "Scan" not "Metadata ingestion"
